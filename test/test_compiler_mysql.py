...
class TestMySql():
    ...
    def clear_db(self) -> None:
        ...
        for tbl in reversed(metadata_obj.sorted_tables):
            # Skip MySQL meta table
            if tbl.name in mysql_meta:
                continue
    ...
    def setup_method(self, method) -> None:
        ...
        self.clear_db()
        self.conn.execute(text("SET GLOBAL sql_mode = 'ANSI_QUOTES'"))
    ...
    def test_unary_rule(self):
        ...
        compiler = Compiler("mysql", self.db_data, Program([rule]))
        compiler.poll()
    ...
    def test_tc_multi_relation(self):
        ...
        compiler = Compiler("mysql", self.db_data, program)
        compiler.poll()
        expected = {
            (1, 2),
            (1, 3),
